...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  const watched = await db.select().from(watchedApprovals).where(and(eq(watchedApprovals.userAddress, owner), eq(watchedApprovals.chainId, chainId)));
  const [user = null] = await db.select().from(watchedUsers).where(eq(watchedUsers.userAddress, owner)).limit(1);
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  const db = await getDb();
  const [existing] = await db.select().from(watchedUsers).where(eq(watchedUsers.userAddress, owner)).limit(1);
  const action = body.action ?? "register";
  if (action === "register") {
    if (!existing) await db.insert(watchedUsers).values({ userAddress: owner, chainId, autoRevokeEnabled: false, createdAt: nowIso() });
    else await db.update(watchedUsers).set({ chainId }).where(eq(watchedUsers.userAddress, owner));
  ...
  if (action === "optInAutoRevoke") {
    ...
    await db.update(watchedUsers).set({ autoRevokeEnabled: true, autoRevokeOptInAt: nowIso(), safeAddress: getAddress(body.safeAddress) }).where(eq(watchedUsers.userAddress, owner));
    return NextResponse.json({ ok: true, autoRevokeEnabled: true });
  }
  await db.update(watchedUsers).set({ autoRevokeEnabled: false, safeAddress: null }).where(eq(watchedUsers.userAddress, owner));
